...
const featureCards: FeatureCard[] = [
  {
    ...
    items: [
      {
        description:
          "Post-Quantum PQC end-to-end encryption for maximum security",
      },
      {
...
function FeatureItemRow({ item }: { item: FeatureItem }) {
  ...
  const rowClassName =
    "flex min-h-[72px] items-start justify-between gap-5 border-t border-[#D7DEE3] py-4";
  ...
  return (
    <Link href={item.href} className={`group ${rowClassName}`}>
      {content}
    </Link>
  );
}
